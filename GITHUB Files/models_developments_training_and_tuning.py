...
entire_feature_set = data_df[['Flow_Type', 'Pipe_Type', 'Years_Since_Last_Inpsection', 'Remaining_Life', 'Pipe_Size', 'Segment_Length', 'Original_Installation ', 'Population_Density','Up_or_Downstream','Flow_Rate']]
...
#%%
feature_imp_RP = pd.Series(risk_prob_results_rf_tuned['model'].feature_importances_, index=risk_prob_features).sort_values(ascending=False)
feature_imp_FI = pd.Series(fail_imp_results_rf_tuned['model'].feature_importances_, index=fail_imp_features).sort_values(ascending=False)
